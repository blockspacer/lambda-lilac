...
namespace lambda
{
	namespace components
	{
		...
		namespace TransformSystem
		{
			...
			{
				for (auto v : scene.transform.unused_data_entries.get_container())
					serializer.serialize("transform/unused_data_entries/", toString(v));
				for (auto v : scene.transform.marked_for_delete)
					serializer.serialize("transform/marked_for_delete/", toString(v));
				for (auto v : scene.transform.data_to_entity)
					serializer.serialize("transform/data_to_entity/", toString(v.first) + "|" + toString(v.second));
				for (auto v : scene.transform.entity_to_data)
					serializer.serialize("transform/entity_to_data/", toString(v.first) + "|" + toString(v.second));

				for (auto v : scene.transform.data)
				{
					String children;
					for (const entity::Entity& child : children)
						children = (children.empty() ? "" : "|") + toString(child);

					serializer.serialize("transform/data/entity/", toString(v.entity));
					serializer.serialize("transform/data/valid/", toString(v.valid));
					serializer.serialize("transform/data/scale/", toString(v.scale.x) + "|" + toString(v.scale.y) + "|" + toString(v.scale.z));
					serializer.serialize("transform/data/translation/", toString(v.translation.x) + "|" + toString(v.translation.y) + "|" + toString(v.translation.z));
					serializer.serialize("transform/data/rotation/", toString(v.rotation.x) + "|" + toString(v.rotation.y) + "|" + toString(v.rotation.z) + "|" + toString(v.rotation.w));
					serializer.serialize("transform/data/children/", children);
				}
			}
...
#pragma optimize ("", off)
			...
			{
				scene.transform.unused_data_entries.get_container().clear();
				scene.transform.marked_for_delete.clear();
				scene.transform.data_to_entity.clear();
				scene.transform.entity_to_data.clear();
				scene.transform.data.clear();

				for (const String& str : serializer.deserializeNamespace("transform/unused_data_entries/"))
					scene.transform.unused_data_entries.push(std::stoul(stlString(str)));
				for (const String& str : serializer.deserializeNamespace("transform/marked_for_delete/"))
					scene.transform.marked_for_delete.insert(std::stoul(stlString(str)));
				for (const String& str : serializer.deserializeNamespace("transform/entity_to_data/"))
					scene.transform.entity_to_data.insert({ std::stoul(stlString(split(str, '|')[0])), std::stoul(stlString(split(str, '|')[1])) });
				for (const String& str : serializer.deserializeNamespace("transform/data_to_entity/"))
					scene.transform.data_to_entity.insert({ std::stoul(stlString(split(str, '|')[0])), std::stoul(stlString(split(str, '|')[1])) });

				Vector<String> entities     = serializer.deserializeNamespace("transform/data/entity/");
				Vector<String> validity     = serializer.deserializeNamespace("transform/data/valid/");
				Vector<String> scales       = serializer.deserializeNamespace("transform/data/scales/");
				Vector<String> translations = serializer.deserializeNamespace("transform/data/translation/");
				Vector<String> rotations    = serializer.deserializeNamespace("transform/data/rotation/");
				Vector<String> children     = serializer.deserializeNamespace("transform/data/children/");
				...
				for (uint32_t i = 0; i < entities.size(); ++i)
				{
					...
					data.rotation = glm::quat(
						std::stof(stlString(split(rotations[i], '|')[3])),
						std::stof(stlString(split(rotations[i], '|')[0])),
						std::stof(stlString(split(rotations[i], '|')[1])),
						std::stof(stlString(split(rotations[i], '|')[2]))
					);
					data.translation = glm::vec3(
						std::stof(stlString(split(translations[i], '|')[0])),
						std::stof(stlString(split(translations[i], '|')[1])),
						std::stof(stlString(split(translations[i], '|')[2]))
					);
					data.scale = glm::vec3(
						std::stof(stlString(split(scales[i], '|')[0])),
						std::stof(stlString(split(scales[i], '|')[1])),
						std::stof(stlString(split(scales[i], '|')[2]))
					);

					for (const String& child : split(children[i], '|'))
						data.children.push_back((entity::Entity)std::stoul(stlString(child)));
					
					data.dirty = true;
					scene.transform.data.push_back(data);
				}
			}
