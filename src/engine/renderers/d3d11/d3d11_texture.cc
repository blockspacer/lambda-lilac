...
namespace lambda
{
	namespace windows
	{
			...
			ID3D11DeviceContext* context)
		{
			for (uint32_t i = 0; i < texture->getLayerCount(); ++i)
			{
				...
				if (layer.isDirty() && !(layer.getFlags() & kTextureFlagFromDDS))
				{
					...
					HRESULT result = context->Map(texture_, i, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource);
		...
		{
			context->PSSetShaderResources(slot, 1u, &srv_);
		}
		...
		{
			LMB_ASSERT(layers_.size() >= layer && layers_[layer].dsvs.size() >= mip_map, "D3D11 TEXTURE: getDSV(Layer: %i, MipMap: %i)", layer, mip_map);
			return layers_[layer].dsvs[mip_map];
		}
			...
			unsigned char mip_map) const
		{
			...
			LMB_ASSERT(layers_.size() >= layer && layers_[layer].rtvs.size() >= mip_map, "D3D11 TEXTURE: getRTV(Layer: %i, MipMap: %i)", layer, mip_map);
			return layers_[layer].rtvs[mip_map];
		}

		///////////////////////////////////////////////////////////////////////////
		const ID3D11Texture2D* D3D11Texture::getTexture() const
		{
			LMB_ASSERT(texture_, "D3D11 TEXTURE: getTexture() : Texture was nullptr");
			return texture_;
		}
		...
		{
			LMB_ASSERT(texture_ != nullptr, "D3D11 TEXTURE: getTexture() : Texture was nullptr");
			return texture_;
		}
			...
			unsigned char mip_count)
		{
			...
			{
				layers_[l].rtvs.resize(mip_count);
			...
			unsigned char mip_count)
		{
			...
			for (unsigned char l = 0u; l < layer_count; ++l)
			{
				layers_[l].dsvs.resize(mip_count);
				for (unsigned char m = 0u; m < mip_count; ++m)
				{
					desc.Texture2DArray.MipSlice = desc.Texture2D.MipSlice = m;
					desc.Texture2DArray.ArraySize = 1;
