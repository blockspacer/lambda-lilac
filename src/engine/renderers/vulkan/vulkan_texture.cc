...
#pragma optimize ("", off)
			...
			, renderer_(renderer)
		{
			...
			// Create the textures.
			{
				result = vkCreateImage(renderer_->getDevice(), &image_create_info, nullptr, &texture_);
			...
			for (uint32_t layer = 0u; layer < image_create_info.arrayLayers && contains_data; ++layer)
			{
				...
				for (uint32_t mip = 0u; mip < image_create_info.mipLevels; ++mip)
				{
					...
					/*VkImageSubDataInfo sub_data_info{};
					...
					result = vkImageSubData(renderer_->getDevice(), texture_, &sub_data_info, data);
		...
		VulkanTexture::~VulkanTexture()
		{
			if (srv_ != VK_NULL_HANDLE)
				vkDestroyImageView(renderer_->getDevice(), srv_, nullptr);

			for (auto& layer : layers_)
				for (auto view : layer)
					vkDestroyImageView(renderer_->getDevice(), view, nullptr);
			layers_.clear();

			if (texture_ != nullptr)
				vkDestroyImage(renderer_->getDevice(), texture_, nullptr);
		}
			...
			unsigned char mip_count)
		{
			...
			VkResult result = vkCreateImageView(renderer_->getDevice(), &image_view_create_info, nullptr, &srv_);
			...
			unsigned char mip_count)
		{
			...
			for (unsigned char l = 0u; l < layer_count; ++l)
			{
				...
				for (unsigned char m = 0u; m < mip_count; ++m)
				{
					...
					result = vkCreateImageView(renderer_->getDevice(), &image_view_create_info, nullptr, &layers_[l][m]);
