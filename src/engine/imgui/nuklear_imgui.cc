...
namespace lambda
{
  namespace imgui
  {
      ...
      command_list_[0].mesh = command_list_[1].mesh = 
        asset::AssetManager::getInstance().createAsset(
          ...
          foundation::Memory::constructShared<asset::Mesh>()
			...
			command_list_[0].shader = asset::ShaderManager::getInstance()->get(Name("resources/shaders/nuklear_imgui.fx"));
      ...
      const platform::WindowMessage& message)
    {
      ...
      {
        nk_input_scroll(context_, nk_vec2(0, (float)(short)message.data[0] / 120.0f));
        break;
      }
