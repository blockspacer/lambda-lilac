...
namespace lambda
{
	namespace gui
	{
		...
		///////////////////////////////////////////////////////////////////////////
		static int CALLBACK matchImprovingEnumProc(
			CONST LOGFONT* candidate, 
			CONST TEXTMETRIC* metrics, 
			DWORD fontType, 
			LPARAM lParam)
		{
			MatchImprovingProcData* matchData = 
				reinterpret_cast<MatchImprovingProcData*>(lParam);
			...
			unsigned chosenWeightDeltaMagnitude = 
				abs(matchData->m_chosen.lfWeight - matchData->m_desiredWeight);
			unsigned candidateWeightDeltaMagnitude = 
				abs(candidate->lfWeight - matchData->m_desiredWeight);

			// If both are the same distance from the desired weight,
			// prefer the candidate if it is further from regular.
			if (chosenWeightDeltaMagnitude == candidateWeightDeltaMagnitude && 
				abs(candidate->lfWeight - FW_NORMAL) > 
				abs(matchData->m_chosen.lfWeight - FW_NORMAL))
			{
				matchData->m_chosen = *candidate;
				return 1;
			}
		...
		///////////////////////////////////////////////////////////////////////////
		ultralight::Ref<ultralight::Buffer> GetFontData(
			const HFONT fontHandle)
		{
		...
		///////////////////////////////////////////////////////////////////////////
		HFONT createGDIFont(
			const WideString& family,
			LONG desiredWeight, 
			bool desiredItalic, 
			int size, 
			bool synthesizeItalic)
		{
			HDC hdc = GetDC(0);
...
#ifdef UNICODE
			WString truncatedFamily = 
				family.substr(0, static_cast<unsigned>(LF_FACESIZE - 1));
			memcpy(
				logFont.lfFaceName, 
				truncatedFamily.data(), 
				truncatedFamily.length() * sizeof(wchar_t)
			);
#else
			Vector<char> family_utf8;
			...
			WideCharToMultiByte(
				...
				-1,
				&family_utf8[0],
				(int)family_utf8.size(), 
				NULL, 
				NULL
			);
			String family_utf8Str = String(&family_utf8[0]);
			String truncatedFamily = 
				family_utf8Str.substr(0, static_cast<unsigned>(LF_FACESIZE - 1));
			memcpy(
				logFont.lfFaceName, 
				truncatedFamily.data(), 
				truncatedFamily.length() * sizeof(char)
			);
#endif
			...
			MatchImprovingProcData matchData(
				toGDIFontWeight(desiredWeight), 
				desiredItalic
			);
			EnumFontFamiliesEx(
				hdc, 
				&logFont, 
				matchImprovingEnumProc, 
		...
		///////////////////////////////////////////////////////////////////////////
		ultralight::Ref<ultralight::Buffer> MyFontLoader::Load(
			...
			float size)
		{
			...
			auto it = fonts_.find(hash);
			if (it == fonts_.end()) 
			{
				fonts_.insert(eastl::make_pair(hash, data));
				return data;
			}
			else 
