...
public:
	...
	int  Open(const lambda::String &filename, const lambda::String &mode);
	int  Close();
	int  GetSize() const;
	bool IsEOF() const;

	// Reading
  lambda::String ReadString(unsigned int length);
  lambda::String ReadLine();
	...
	// Writing
	int WriteString(const lambda::String &str);
