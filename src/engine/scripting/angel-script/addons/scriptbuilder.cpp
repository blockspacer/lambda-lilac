...
// Helper functions
static lambda::String GetCurrentDir();
static lambda::String GetAbsolutePath(const lambda::String &path);
...
lambda::String CScriptBuilder::GetSectionName(unsigned int idx) const
{
	if( idx >= includedScripts.size() ) return "";

#ifdef _WIN32
  lambda::Set<lambda::String, ci_less>::const_iterator it = includedScripts.begin();
#else
  lambda::Set<lambda::String>::const_iterator it = includedScripts.begin();
#endif
	while( idx-- > 0 ) it++;
	return *it;
}
...
int CScriptBuilder::AddSectionFromFile(const char *filename)
{
	...
	lambda::String fullpath = GetAbsolutePath(filename);
...
{
	lambda::String scriptFile = filename;
...
int CScriptBuilder::LoadScriptSection(const char *filename)
{
	...
	if( f == 0 )
	{
		...
		lambda::String msg = "Failed to open script file '" + GetAbsolutePath(scriptFile) + "'";
	...
	{
		// Write a message to the engine's message callback
		lambda::String msg = "Failed to load script file '" + GetAbsolutePath(scriptFile) + "'";
		engine->WriteMessage(filename, 0, 0, asMSGTYPE_ERROR, msg.c_str());
		return -1;
	}
...
{
	lambda::Vector<lambda::String> includes;
...
#if AS_PROCESS_METADATA == 1
	// Preallocate memory
	lambda::String metadata, declaration;
	...
	while( pos < modifiedScript.size() )
	{
		...
		if( modifiedScript[pos] == '[' )
		{
			// Get the metadata lambda::String
	...
	if( includes.size() > 0 )
	{
		...
		else
		{
			...
			lambda::String path = sectionname;
			size_t posOfSlash = path.find_last_of("/\\");
			if( posOfSlash != lambda::String::npos )
			...
			for( int n = 0; n < (int)includes.size(); n++ )
			{
				...
				if( includes[n].find_first_of("/\\") != 0 &&
					includes[n].find_first_of(":") == lambda::String::npos )
				{
...
int CScriptBuilder::Build()
{
...
#if AS_PROCESS_METADATA == 1
	// After the script has been built, the metadata lambda::Strings should be
	...
	for( int n = 0; n < (int)foundDeclarations.size(); n++ )
	{
		...
		if( decl->type == 1 )
		{
			...
			assert( typeId >= 0 );
			if( typeId >= 0 )
				typeMetadataMap.insert(lambda::Map<int, lambda::String>::value_type(typeId, decl->metadata));
		}
		else if( decl->type == 2 )
		{
			if( decl->parentClass == "" )
			{
				...
				assert( func );
				if( func )
					funcMetadataMap.insert(lambda::Map<int, lambda::String>::value_type(func->GetId(), decl->metadata));
			}
			else
			{
				// Find the method id
				...
				assert( typeId > 0 );
        lambda::Map<int, SClassMetadata>::iterator it = classMetadataMap.find(typeId);
				if( it == classMetadataMap.end() )
				{
					classMetadataMap.insert(lambda::Map<int, SClassMetadata>::value_type(typeId, SClassMetadata(decl->parentClass)));
				...
				assert( func );
				if( func )
					it->second.funcMetadataMap.insert(lambda::Map<int, lambda::String>::value_type(func->GetId(), decl->metadata));
			}
		}
		else if( decl->type == 4 )
		{
		...
		else if( decl->type == 3 )
		{
			if( decl->parentClass == "" )
			{
				...
				assert( varIdx >= 0 );
				if( varIdx >= 0 )
					varMetadataMap.insert(lambda::Map<int, lambda::String>::value_type(varIdx, decl->metadata));
			}
			else
			{
				int typeId = module->GetTypeIdByDecl(decl->parentClass.c_str());
        ...
        lambda::Map<int, SClassMetadata>::iterator it = classMetadataMap.find(typeId);
				...
				// If found, add it
				assert( idx >= 0 );
				if( idx >= 0 ) it->second.varMetadataMap.insert(lambda::Map<int, lambda::String>::value_type(idx, decl->metadata));
			}
		}
	}
...
#if AS_PROCESS_METADATA == 1
int CScriptBuilder::ExtractMetadataString(int pos, lambda::String &metadata)
{
	metadata = "";
...
int CScriptBuilder::ExtractDeclaration(int pos, lambda::String &declaration, int &type)
{
	declaration = "";
	type = 0;
...
{
  lambda::Map<int,lambda::String>::iterator it = typeMetadataMap.find(typeId);
...
const char *CScriptBuilder::GetMetadataStringForFunc(asIScriptFunction *func)
{
	if( func )
	{
    lambda::Map<int,lambda::String>::iterator it = funcMetadataMap.find(func->GetId());
...
{
  lambda::Map<int,lambda::String>::iterator it = varMetadataMap.find(varIdx);
...
{
  lambda::Map<int, SClassMetadata>::iterator typeIt = classMetadataMap.find(typeId);
	if(typeIt == classMetadataMap.end()) return "";

  lambda::Map<int, lambda::String>::iterator propIt = typeIt->second.varMetadataMap.find(varIdx);
...
const char *CScriptBuilder::GetMetadataStringForTypeMethod(int typeId, asIScriptFunction *method)
{
    ...
    lambda::Map<int, lambda::String>::iterator methodIt = typeIt->second.funcMetadataMap.find(method->GetId());
...
lambda::String GetAbsolutePath(const lambda::String &file)
{
	lambda::String str = file;

	// If this is a relative path, complement it with the current path
	if( !((str.length() > 0 && (str[0] == '/' || str[0] == '\\')) ||
		  str.find(":") != lambda::String::npos) )
	{
	...
	size_t pos = 0;
	while( (pos = str.find("\\", pos)) != lambda::String::npos )
		str[pos] = '/';
	...
	pos = 0;
	while( (pos = str.find("/../")) != lambda::String::npos )
	{
		size_t pos2 = str.rfind("/", pos-1);
		if( pos2 != lambda::String::npos )
...
lambda::String GetCurrentDir()
{
	...
	if (!apppath[0])
	{
		...
		// Terminate the lambda::String after the trailing backslash
