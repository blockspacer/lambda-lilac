...
public:
	CSerializedValue();
	CSerializedValue(CSerializedValue *parent, const String &name, const String &nameSpace, void *ref, int typeId);
	~CSerializedValue();
	...
	CSerializedValue *FindByName(const String &name, const String &nameSpace);
	...
	Vector<CSerializedValue*> m_children;
...
protected:
	...
	void  GetAllPointersOfChildren(Vector<void*> *ptrs);
...
public:
	...
	void AddUserType(CUserType *ref, const String &name);
...
protected:
	...
	Map<String, CUserType*> m_userTypes;
	...
	Vector<SExtraObject> m_extraObjects;
};
