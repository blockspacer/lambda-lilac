...
CSerializer::~CSerializer()
{
	...
	Map<String, CUserType*>::iterator it;
...
void CSerializer::AddUserType(CUserType *ref, const String &name)
{
...
int CSerializer::Store(asIScriptModule *mod)
{
	...
	for( i = 0; i < mod->GetGlobalVarCount(); i++ )
	{
		...
		m_root.m_children.push_back(foundation::Memory::construct<CSerializedValue>(&m_root, name, nameSpace, mod->GetAddressOfGlobalVar(i), typeId));
	}

	// Second store extra objects
	for( i = 0; i < m_extraObjects.size(); i++ )
		m_root.m_children.push_back(foundation::Memory::construct<CSerializedValue>(&m_root, "", "", m_extraObjects[i].originalObject, m_extraObjects[i].originalTypeId));
...
CSerializedValue::CSerializedValue(CSerializedValue *parent, const String &name, const String &nameSpace, void *ref, int typeId) 
{
	Init();
...
void CSerializedValue::ClearChildren()
{
    ...
    foundation::Memory::destruct(m_children[n]);
	m_children.clear();
}
...
CSerializedValue *CSerializedValue::FindByName(const String &name, const String &nameSpace)
{
...
void  CSerializedValue::GetAllPointersOfChildren(Vector<void*> *ptrs)
{
...
void CSerializedValue::Store(void *ref, int typeId)
{
	...
	else if( m_typeId & asTYPEID_SCRIPTOBJECT )
	{
		...
		{	
			...
			m_children.push_back(foundation::Memory::construct<CSerializedValue>(this, childName, "", obj->GetAddressOfProperty(i), childId));
		}	
	}
	else
	{
...
void CSerializedValue::Restore(void *ref, int typeId)
{
	...
	else
	{
		...
		{
			String str = "Cannot restore type '";
			str += type->GetName();
			str += "'";
...
void CSerializedValue::ReplaceHandles()
{
	if( m_handlePtr )
	{
		...
		if( handle_to == 0 )
		{
			...
			asITypeInfo *type = GetType();
			CSerializedValue *need_create = foundation::Memory::construct<CSerializedValue>(this, m_name, m_nameSpace, m_handlePtr, type->GetTypeId());
