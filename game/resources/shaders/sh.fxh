...
float3 EvalSH(const in float3 dir, const in float3 coefficients[9], const in float3 transfer_func)
{
	const float3 r0 = float3(coefficients[0].r, coefficients[1].r, coefficients[2].r);
	const float3 r1 = float3(coefficients[3].r, coefficients[4].r, coefficients[5].r);
	const float3 r2 = float3(coefficients[6].r, coefficients[7].r, coefficients[8].r);
	const float3 g0 = float3(coefficients[0].g, coefficients[1].g, coefficients[2].g);
	const float3 g1 = float3(coefficients[3].g, coefficients[4].g, coefficients[5].g);
	const float3 g2 = float3(coefficients[6].g, coefficients[7].g, coefficients[8].g);
	const float3 b0 = float3(coefficients[0].b, coefficients[1].b, coefficients[2].b);
	const float3 b1 = float3(coefficients[3].b, coefficients[4].b, coefficients[5].b);
	const float3 b2 = float3(coefficients[6].b, coefficients[7].b, coefficients[8].b);

	const float sqrt3 = 1.732050808;

	// rotate ZH coef to world space SH in spherical coordinates (i.e. z-axiz is up axis)
	const float3 t0 = float3(
		 transfer_func.x, 
		-transfer_func.y * dir.y, 
		 transfer_func.y * dir.z
	);
	const float3 t1 = float3(
		        -transfer_func.y * dir.x,
		 sqrt3 * transfer_func.z * dir.x * dir.y,
		-sqrt3 * transfer_func.z * dir.y * dir.z
	);
	const float3 t2 = float3(
		0.5    *         transfer_func.z * (3.0 * dir.z * dir.z - 1.0), 
		-sqrt3 *         transfer_func.z *  dir.x * dir.z, 
		0.5    * sqrt3 * transfer_func.z * (dir.x * dir.x - dir.y * dir.y)
	);
	
	return float3(
	    dot(r0, t0) + dot(r1, t1) + dot(r2, t2),
		dot(g0, t0) + dot(g1, t1) + dot(g2, t2), 
		dot(b0, t0) + dot(b1, t1) + dot(b2, t2)
	);
}